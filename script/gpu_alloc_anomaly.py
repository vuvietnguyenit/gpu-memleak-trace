    ...
    for it in range(args.iterations):
        ...
        try:
            ...
            if device.type == "cuda":
                ...
                log_cuda_mem(f"T{tid} iter {it} ({'ANOMALY' if is_anomaly else 'normal'} {size_mb} MB)", device)
            ...
            msg = str(e)
            print(f"[T{tid} iter {it}] RuntimeError: {msg}", file=sys.stderr, flush=True)
...
def main():
    p = argparse.ArgumentParser(description="GPU allocation anomaly simulator with PyTorch")
    p.add_argument("--device", default="cuda:0", help="Device to use, e.g., cuda:0 or cpu")
    p.add_argument("--threads", type=int, default=4, help="Number of worker threads (simulated concurrent streams)")
    p.add_argument("--iterations", type=int, default=20, help="Iterations (batches) per thread")
    p.add_argument("--normal-mb", type=float, default=128, help="Normal per-iteration allocation size in MB")
    p.add_argument("--anomaly-mb", type=float, default=4096, help="Anomalous allocation size in MB for one thread/iter")
    p.add_argument("--anomaly-thread", type=int, default=0, help="Thread index that will perform the anomalous allocation")
    p.add_argument("--anomaly-iter", type=int, default=5, help="Iteration at which anomaly happens (0-based)")
    p.add_argument("--hold-refs", type=int, default=3, help="How many allocations to keep referenced per thread to simulate retention/leaks (0 disables)")
    p.add_argument("--sleep-ms", type=int, default=50, help="Sleep between iterations to control pacing")
    p.add_argument("--use-model", action="store_true", help="Run a tiny NN on the allocated tensor to add compute load")
    p.add_argument("--model-hidden", type=int, default=1024, help="Hidden size for the tiny model (only if --use-model)")
    p.add_argument("--keep-intermediate", action="store_true", help="Keep model outputs referenced (increases retention)")
    p.add_argument("--reset-peak-each-iter", action="store_true", help="Reset CUDA peak memory stats after each iteration")
    p.add_argument("--stop-on-oom", action="store_true", help="Stop all threads when an OOM occurs")
    args = p.parse_args()

    device = torch.device(args.device)
    if device.type == "cuda":
        if not torch.cuda.is_available():
            print("CUDA is not available. Use --device cpu or install CUDA-enabled PyTorch.", file=sys.stderr)
            sys.exit(1)
        # Warm up context
        torch.cuda.init()
        torch.cuda.reset_peak_memory_stats(device)
    ...
    barrier = threading.Barrier(args.threads + 1)  # +1 for main thread to synchronize starts
    stop_flag = threading.Event()
    hold_refs_lists: List[List[torch.Tensor]] = [[] for _ in range(args.threads)]

    threads = []
    for tid in range(args.threads):
        t = threading.Thread(
            target=worker,
            args=(tid, barrier, args, stop_flag, hold_refs_lists, device, model),
            daemon=True,
        )
        t.start()
        threads.append(t)
